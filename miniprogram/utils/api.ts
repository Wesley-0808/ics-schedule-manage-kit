export const BASE_URL = 'http://localhost:8000'; // 后端服务地址
...
export const api = {
  ...
  getCalendar: (calendarId: string) => request<any>(`/calendars/${calendarId}`, 'GET'),
  deleteCalendar: (calendarId: string) => request<any>(`/calendars/${calendarId}`, 'DELETE'),
  ...
  getCalendarEvents: (calendarId: string, params?: any) =>
    request<{ events: any[] }>(`/calendars/${calendarId}/events`, 'GET', params),
  ...
  getCategories: (calendarId: string) =>
    request<{ categories: any[] }>(`/calendars/${calendarId}/categories`, 'GET'),
  ...
  previewRrule: (data: any) => request<any>('/utils/rrule/preview', 'POST', data),
